...
local function checkStatus()
    ...
    if response == "UPDATE" then
        if not CheckData.kicked then
            ...
            LocalPlayer:Kick("ManHub Update,Rejoin Ulang Dan Execute Lagi!.")
        end
    elseif response == "LATEST" then
        if not CheckData.notified then
            CheckData.notified = true
            warn("[ManHub] Status: Latest version")
        end
    else
        warn("[ManHub] Status unknown:", response)
    end
end
...
WindUI:Popup({
    ...
    Icon = "crown",
    Content = [[
Thank you for using Man Hub.
Don't forget to Join Man Hub Community!
]],
    Buttons = {
